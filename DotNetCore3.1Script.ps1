...
mkdir Lambda
cd Lambda
dotnet new webapi -f netcoreapp3.1 --name "$projectName.Lambda"
dotnet new classlib -f netcoreapp3.1 --name "$projectName.Lambda.Interface"
dotnet new classlib -f netcoreapp3.1 --name "$projectName.Lambda.ViewModel"
cd ..

mkdir Application
cd Application
dotnet new classlib -f netcoreapp3.1 --name "$projectName.Application"
dotnet new classlib -f netcoreapp3.1 --name "$projectName.Application.Infrastructure"
dotnet new classlib -f netcoreapp3.1 --name "$projectName.Application.Interface"
cd ..

mkdir Domain
cd Domain
dotnet new classlib -f netcoreapp3.1 --name "$projectName.Domain"
dotnet new classlib -f netcoreapp3.1 --name "$projectName.Domain.Infrastructure"
cd ..

mkdir Repository
cd Repository
dotnet new classlib -f netcoreapp3.1 --name "$projectName.Repository"
dotnet new classlib -f netcoreapp3.1 --name "$projectName.Repository.Infrastructure"
dotnet new classlib -f netcoreapp3.1 --name "$projectName.Repository.Interface"
cd ..

mkdir Test
cd Test
dotnet new nunit -f netcoreapp3.1 --name "$projectName.Application.Test"
dotnet new nunit -f netcoreapp3.1 --name "$projectName.Domain.Infrastructure.Test"
dotnet new nunit -f netcoreapp3.1 --name "$projectName.Domain.Test"
dotnet new nunit -f netcoreapp3.1 --name "$projectName.Lambda.Test"
dotnet new nunit -f netcoreapp3.1 --name "$projectName.Repository.Test"
...
dotnet add Application\$projectName.Application\$projectName.Application.csproj reference Domain\$projectName.Domain\$projectName.Domain.csproj
...
dotnet add package Microsoft.AspNetCore.Mvc.NewtonsoftJson --version 3.1.19
...
namespace ' + $projectName + '.Lambda
{
    ...
    public class Startup
    {
        ...
        private static string EnvironmentName => Environment.GetEnvironmentVariable(@"EnvironmentName") ?? @"development";

        // This method gets called by the runtime. Use this method to add services to the container
        public void ConfigureServices(IServiceCollection services)
        {
            services.AddProxyServiceCollectionLibrary();
            services.AddLogging(SetupLogger);

            services.AddHttpClient("RestEndpointInvokerService", client => { client.Timeout = TimeSpan.FromMilliseconds(25000); });
            services.AddAutoMapper(typeof(Startup));

            services
                .AddMvcCore(options => options.EnableEndpointRouting = false)
                .AddCors(co => co.AddPolicy(CorsPolicy,
                             cp =>
                             {
                                 cp.AllowAnyHeader();
                                 cp.AllowAnyMethod();
        ...
        private static void SetupLogger(ILoggingBuilder logging)
        {
            if (logging == null)
            {
            ...
            if (WebHostEnvironment.EnvironmentName.Equals(@"development", StringComparison.InvariantCultureIgnoreCase))
            {
                logging.AddConsole();
            }
        }
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            ...
            app.UseSwagger();
            app.UseSwaggerUI(c => { c.SwaggerEndpoint("../swagger/v1/swagger.json", "AO Last Mile Track Your Order ETA API"); });
